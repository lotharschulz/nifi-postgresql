...
- **File Structure:**
  ...
  - `monitor-cdc-slot.sh` – CDC slot monitoring with WAL lag detection (you READ/WRITE here)
...
- **Diagnose system:** `./nifi-diagnose.sh` (checks NiFi, PostgreSQL, and flow status with CDC slot monitoring)
- **Monitor CDC slots:** `./monitor-cdc-slot.sh` (checks replication slot status and WAL lag; warns when lag > 500 MB)
  - `./monitor-cdc-slot.sh --continuous` (monitors every 10 seconds)
  - `./monitor-cdc-slot.sh --continuous --interval=30` (monitors every 30 seconds)
  - `./monitor-cdc-slot.sh --help` (shows usage and best practices)
