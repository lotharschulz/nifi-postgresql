...
echo -e "${GREEN}Starting NiFi CDC Pattern Setup (Fixed Version)...${NC}"
...
create_process_group() {
    ...
    curl -sk -X POST "${NIFI_URL}/nifi-api/process-groups/${parent_id}/process-groups" \
        ...
        -d "{\"revision\":{\"version\":0},\"component\":{\"name\":\"${name}\",\"position\":{\"x\":100,\"y\":100}}}" | jq -r '.id'
}
...
main() {
    ...
    PG_ID=$(curl -sk "${NIFI_URL}/nifi-api/flow/process-groups/root" \
        -H "Authorization: Bearer ${TOKEN}" | \
        jq -r '.processGroupFlow.flow.processGroups[]? | select(.component.name=="PostgreSQL CDC Pattern") | .component.id' | head -1)
    ...
    PARAM_CTX_ID=$(curl -sk "${NIFI_URL}/nifi-api/flow/parameter-contexts" \
        -H "Authorization: Bearer ${TOKEN}" | \
        jq -r '.parameterContexts[]? | select(.component.name=="CDC-DB") | .id' | head -1)
    
    if [ -z "$PARAM_CTX_ID" ] || [ "$PARAM_CTX_ID" = "null" ]; then
        PARAM_CTX_ID=$(curl -sk -X POST "${NIFI_URL}/nifi-api/parameter-contexts" \
            -H "Authorization: Bearer ${TOKEN}" \
            -H "Content-Type: application/json" \
            -d "{\"revision\":{\"version\":0},\"component\":{\"name\":\"CDC-DB\",\"parameters\":[
    ...
    # 1. ExecuteSQL - Query replication slot
    EXEC_SQL_ID=$(create_processor "$PG_ID" "org.apache.nifi.processors.standard.ExecuteSQL" "Read CDC Slot" 400 100)
    EXEC_SQL_CONFIG=$(cat <<EOF
{
    "properties": {
...
EOF
    ...
    # 3. SplitJson
    SPLIT_ID=$(create_processor "$PG_ID" "org.apache.nifi.processors.standard.SplitJson" "Split Changes" 400 400)
...
EOF
    ...
    # 4. EvaluateJsonPath
    EVAL_ID=$(create_processor "$PG_ID" "org.apache.nifi.processors.standard.EvaluateJsonPath" "Parse CDC Data" 400 550)
    EVAL_CONFIG=$(cat <<EOF
{
    "properties": {
...
EOF
    ...
    # 5. RouteOnAttribute
    ROUTE_ID=$(create_processor "$PG_ID" "org.apache.nifi.processors.standard.RouteOnAttribute" "Route Changes" 400 700)
...
EOF
    ...
    configure_processor "$ROUTE_ID" "$ROUTE_CONFIG" "RouteOnAttribute"
    
    # 6. LogAttribute
    LOG_ID=$(create_processor "$PG_ID" "org.apache.nifi.processors.standard.LogAttribute" "Log CDC Changes" 400 850)
    LOG_CONFIG=$(cat <<EOF
{
    "properties": {
        "Log Level": "info",
...
EOF
    ...
    create_connection "$EXEC_SQL_ID" "success" "$CONVERT_ID" "$PG_ID"
    create_connection "$CONVERT_ID" "success" "$SPLIT_ID" "$PG_ID"
    create_connection "$SPLIT_ID" "split" "$EVAL_ID" "$PG_ID"
    create_connection "$EVAL_ID" "matched" "$ROUTE_ID" "$PG_ID"
    create_connection "$ROUTE_ID" "has_changes" "$LOG_ID" "$PG_ID"
    ...
    echo -e "1. Create replication slot (if not exists):"
    echo -e "   ${BLUE}./test-cdc.sh --setup${NC}"
    echo -e "2. Start the flow in NiFi UI"
    echo -e "3. Generate test data:"
    echo -e "   ${BLUE}./test-cdc.sh${NC}"
}
